...
import type { Transaction, Currency, Country, AssetClass } from "@/lib/types";
...
export async function refreshLivePrices(force = false) {
  ...
  // Dedup holdings by ticker+currency — matches latest_prices' unique key.
  const holdings = new Map<string, HoldingKey>();
  for (const t of transactions) {
    const ticker = t.asset_ticker.trim().toUpperCase();
