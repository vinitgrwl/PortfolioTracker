...
-- ---------------------------------------------------------------------
-- company_events: renames and mergers — a company that changed its
-- ticker/ISIN (e.g. Zomato -> Eternal) or was absorbed into another
-- listed company (e.g. HDFC -> HDFC Bank, Mindtree -> LTIMindtree).
-- A rename is stored as a merger with ratio 1:1 — same transform either
-- way. Applied only to computed views (holdings, realized P&L, price
-- lookups) via lib/companyEvents.ts — the raw Transactions ledger always
-- shows what was actually logged at the time, untouched.
--
-- A straight buyout for cash (e.g. Twitter/TWTR going private) is NOT
-- modeled here — there's no reliable way to know the exact payout price
-- automatically, so that's just logged as a normal Sell transaction at
-- whatever price/date the user actually received.
-- ---------------------------------------------------------------------
create table if not exists company_events (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,

  event_type text not null check (event_type in ('rename', 'merger')),

  old_ticker text not null,
  old_isin text,
  old_country text not null check (old_country in ('United States', 'India')),

  new_ticker text not null,
  new_isin text,
  new_country text not null check (new_country in ('United States', 'India')),

  ratio_from numeric not null default 1 check (ratio_from > 0), -- old shares held
  ratio_to numeric not null default 1 check (ratio_to > 0),     -- new shares received per ratio_from old
  effective_date date not null,

  created_at timestamptz not null default now(),

  unique (user_id, old_ticker, old_country, effective_date)
);

alter table company_events enable row level security;

drop policy if exists "company_events_owner_all" on company_events;
create policy "company_events_owner_all" on company_events
  for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

create index if not exists company_events_old_ticker_idx on company_events(old_ticker);
